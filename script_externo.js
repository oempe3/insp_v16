...
function handleReportSubmit() {
  ...
  const dataToSend = { ...newInspectionData };
    ...
    .then(resultText => {
      ...
      try {
        ...
        if (parsed && typeof parsed === 'object') {
          ...
          mensagemServidor = parsed.message || '';
        }
      } catch (e) {
        // Se não for JSON, mantém resultText original (compatível com versão antiga que devolve "ok").
      }

      // Guarda cópia da inspeção atual em 'previous' e limpa o restante
      ...
      const di = inspectionData.previous['dados-iniciais'] || {};
      lastNames = {
        operador: di.operador || '',
        supervisor: di.supervisor || ''
      };
      saveLastNames(lastNames);

      // Monta texto padrão para WhatsApp
      const dadosIniciais = inspectionData.previous['dados-iniciais'] || {};
      const data = dadosIniciais.data || '';
      const operador = dadosIniciais.operador || '';
      const supervisor = dadosIniciais.supervisor || '';
      const turma = dadosIniciais.turma || '';
      const turno = dadosIniciais.turno || '';

      let textoBase = 'Relatório de inspeção EXTERNA – ' + data + '\n' +
        'Operador: ' + operador + '\n';
      ...
      const compartilhar = pdfUrl
        ? confirm('✅ Relatório enviado com sucesso!\n\nDeseja compartilhar o PDF pelo WhatsApp agora?')
        : false;
      ...
      alert('✅ Relatório enviado com sucesso! ' + (mensagemServidor || 'O formulário foi limpo para uma nova inspeção.'));
...
async function carregarUltimaInspecaoExterna() {
  try {
    ...
    if (!json.success || !json.data) {
      alert(json.message || 'Não foi possível localizar a última inspeção externa.');
      return;
    }

    aplicarUltimaInspecaoInterna(json.data);
    alert(
      'Dados da inspeção externa anterior carregados com sucesso. Revise e ajuste antes de enviar o relatório.'
    );
  } catch (e) {
    if (typeof hideSpinner === 'function') {
      hideSpinner();
    }
    console.error('Erro ao carregar última inspeção externa:', e);
    alert('Erro ao carregar dados da inspeção externa: ' + e.message);
  }
}
